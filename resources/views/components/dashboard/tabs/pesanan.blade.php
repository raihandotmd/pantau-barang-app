...
<div x-show="activeTab === 'pesanan'" class="bg-white rounded-lg shadow-sm border border-gray-100 p-6" style="display: none;">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <form method="GET" action="{{ route('dashboard') }}" class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            <div class="relative">
                <input type="text" name="search_order" value="{{ request('search_order') }}" placeholder="Cari pelanggan..." class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-pantau-medium focus:border-pantau-medium w-full sm:w-64">
            ...
            <select name="status_filter" onchange="this.form.submit()" class="border border-gray-300 rounded-lg text-sm focus:ring-pantau-medium focus:border-pantau-medium py-2 pl-3 pr-8">
                ...
                @forelse($recentOrders as $order)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button x-data="" x-on:click.prevent="$dispatch('open-modal', { name: 'order-detail', order: {{ $order }} })" class="text-pantau-medium hover:text-pantau-dark">Detail</button>
                        </td>
                    </tr>
                @empty
                    <tr>
